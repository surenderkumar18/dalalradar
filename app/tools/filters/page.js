...
/*
  function extractLabelFromFile(fileName) {
    if (!fileName) return "";

    const match = fileName.match(/(\d{8})/);
    if (!match) return "";

    const dateStr = match[1]; // 20260323
    ...
    return `${monthMap[mm]}_${Number(dd)}`;
  }
*/
...
export default function FiltersPage() {
  ...
  const fetchDeliveryFiles = async () => {
    ...
    const filtered = allFiles.filter((file) =>
      /^sec_bhavdata_full_\d{8}\.csv$/i.test(file),
    );

    setCsvFiles(filtered);
  };

  const handleProcessOiCSV = async () => {
    const res = await fetch("/api/process-oi-csv", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fileName: selectedOiFile }),
    });
    ...
    if (!data.parsedData || data.parsedData.length === 0) {
      alert("❌ No OI data found in this file.");
      return;
    }

    setOiData(data.parsedData);
    setOiRowCount(data.rowCount);
    setNewLabel(""); // reset label

    if (data.lowCountWarning) {
      setOiWarningState("low");
    } else {
      setIsOiModalOpen(true); // ✅ Correct modal
    }
  };

  const handleSaveOi = async () => {
    if (!oiData || oiData.length === 0) {
      alert("No OI data to save");
      return;
    }
    ...
    const res = await fetch("/api/save-oi-data", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    alert("Saved Successfully 5555✅");

    setOiData(null);
    setSelectedOiFile("");
    setNewLabel("");
    setIsOiModalOpen(false);
  };
  ...
  const handleProcessCSV = async () => {
    ...
    // 🔥 STOP if no mapped data
    if (!data.mappedData || Object.keys(data.mappedData).length === 0) {
      alert("❌ No F&O EQ stocks found in this file.");
      return;
    }

    setMappedData(data.mappedData);
    setNewLabel(data.suggestedLabel);

    if (data.lowCountWarning || data.alreadyExists) {
      setWarningState(data);
    } else {
      setIsDeliveryModalOpen(true); // ✅ Correct modal
    }
  };
...
*/
  const handleProcessDeliveryCSV = async () => {
    ...
    setLoadingDelivery(true); // ✅ START LOADING
    ...
    try {
      ...
      setLoadingDelivery(false); // ✅ END LOADING
    } catch (err) {
  ...
  // ✅ Helper to convert textarea input to array
  const parseArray = (input) => {
    ...
    return arr
      .map((item) => item.toString().trim().toUpperCase()) // 🔥 normalize
      .filter((item) => item.length > 0);
  };

  const handleProcessFnoCSV = async () => {
    /**
     * Select Date (DayPicker)
      ↓
    handleProcessFnoCSV()
      ↓
    /api/download-fno
  ...
  const handleSaveFno = async () => {
    ...
    const resSave = await fetch("/api/save-fno-data", {
      ...
      body: JSON.stringify({
        label: newLabel.toLowerCase(),
        data: fnoData, // ✅ USE EXISTING DATA
      }),
    });
  ...
  const handleOutputPaste = (value) => {
    try {
      const parsed = JSON.parse(value);
      setJsonMinified(JSON.stringify(parsed)); // minified in one-line
    } catch {
      setJsonMinified("❌ Invalid JSON");
    }
  };
  ...
  const handleFilterEQSeries = (value) => {
    ...
    try {
      ...
      // ✅ Store clean object for SAVE
      setMappedData(mapped);

      // ✅ Show pretty output in textarea
      setEqJsonOutput(JSON.stringify(mapped, null, 2));

      // ✅ Count of mapped F&O EQ stocks
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setTimeout(() => {
          setIsDeliveryModalOpen(false); // ✅ correct
          setSaveSuccess("");
        }, 1200);
      }
    } catch {
  ...
  return (
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <div className="space-y-16">
          ...
          <section>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="mb-2 font-medium">Paste JSON Object</h3>
                <textarea
                  id="textarea-json-input"
                  value={jsonInput}
                  onChange={(e) => handleMinifyJson(e.target.value)}
                  placeholder='Paste JSON like {"name": "A", "age": 25}'
                  className="w-full h-32 bg-gray-800 border border-gray-700 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/40 rounded-lg p-3 text-sm font-mono resize-none outline-none transition"
                />
              </div>

              <div>
                <h3 className="mb-2 font-medium">Minified JSON (Readonly)</h3>
                <textarea
                  id="textarea-json-output"
                  value={jsonMinified}
                  onChange={(e) => handleOutputPaste(e.target.value)}
    ...
    px-6 py-3 rounded-lg text-lg font-semibold transition shadow-md"
                  >
                    {loadingDelivery
                      ? "Processing..."
